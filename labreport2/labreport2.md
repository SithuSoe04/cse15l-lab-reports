...
```
The following picture shows the symptom of running the test in the first code snippet:
![test-filter](test-filter.png)
As you can see, we failed the test because we added an input that induces a failure. 

The following picture shows the symptom of running the test in the second code snippet:
![test-filter](test-filter.png)
As you can see, we passed the test because we added an input that does not induce a failure. Two tests were run, but only one failed. That failed test was the one above. 

The following code snippet shows the code before the bug was fixed:
```
